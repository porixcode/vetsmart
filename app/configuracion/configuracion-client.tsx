...
const NAV: NavGroup[] = [
  { group: "PERFIL DE LA CLÍNICA", items: [
...
const SECTION_META: Partial<Record<SectionId, { title: string; description: string }>> = {
  ...
  integrations:       { title:"Integraciones", description:"Conecta con servicios externos." },
...
export function ConfiguracionClient({ clinicInfo, schedule }: ConfiguracionClientProps) {
  const [activeSection, setActiveSection] = React.useState<SectionId>("general")

  const renderSection = (id: SectionId) => {
    if (id === "general")  return <SectionGeneral clinicInfo={clinicInfo} />
    if (id === "schedule") return <SectionSchedule schedule={schedule} />
            ...
            {group.items.map(item => {
              ...
              return (
                <button key={item.id} onClick={() => setActiveSection(item.id)}
                  className={cn("flex w-full items-center gap-2.5 px-4 py-2 text-sm transition-colors relative",
                    isActive ? "text-primary font-medium bg-primary/5 before:absolute before:left-0 before:top-1 before:bottom-1 before:w-0.5 before:rounded-r before:bg-primary"
